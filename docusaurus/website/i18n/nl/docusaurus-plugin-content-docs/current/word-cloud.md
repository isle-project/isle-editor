...
* __fontSizeMapper__ | `function`: accessor functie die de lettergrootte voor elk woord aangeeft. Standaard is dit `( count - min + 1 ) / ( max - min + 6 ) * 30.0 + 14.0`, waarbij `min` en `max` respectievelijk het minimum en maximum woordaantal aangeven.. Default: `none`.
