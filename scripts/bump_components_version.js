...
const pkgPath = join( __dirname, '..', '@isle-project', 'components', 'package.json' );
...
fs.readdir( join( __dirname, '..', '@isle-project', 'components', 'dist' ), ( error, files ) => {
	if ( error ) {
		throw error;
	}
	files.filter( name => RE_DLL_FILE.test( name ) ).forEach( x => {
		fs.unlink( join( __dirname, '..', '@isle-project', 'components', 'dist', x ), ( err ) => {
			if ( err ) {
				console.error( err );
			}
		});
