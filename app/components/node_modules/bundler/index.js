...
const getSessionCode = () => {
	...
	str += 'global.session = new Session( global.ISLE );';
	return str;
};
...
function writeIndexFile( outputPath, lessonContent, meta, minify, clbk ) {
	...
	const cssOutputPath = path.join( appDir, 'lesson.css' );
	const bundlePath = path.join( appDir, 'bundle.js' );
	const cssPath = isPackaged ?
		`${process.resourcesPath}/app/app/css/lesson.css` :
		path.resolve( './app/css/lesson.css' );
	...
	fs.copySync( cssPath, cssOutputPath );
