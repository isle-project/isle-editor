      ...
      <ul style={styles.container}>
        { 
          this.options.map((q, i) => {
            ...
            style = match[0] ? Object.assign({}, style, { backgroundColor: match[0].color }) : style;
...
class MatchListQuestion extends Component{
  ...
  constructor(props) {
    super(props);
    const { questions, colorScale } = this.props;
    this.state = {
      selectedA: null,
      selectedB: null,
      colorScale,
  ...
  componentDidUpdate() {
    ...
    if (selectedA && selectedB) {
      answers.push({ a: selectedA, b: selectedB, color: colorScale.pop() });
      this.setState({ selectedA: null, selectedB: null, answers });
    }
  }
    ...
    return (
      <div style={styles.container}>
        <div style={styles.lists}>
          <OptionsList 
            options={questions.map(q => q.a)} 
            onSelect={onSelectA}
            answers={answers}
            active={this.state.selectedA} />
          <OptionsList 
            options={questions.map(q => q.b)} 
            onSelect={onSelectB}
            answers={answers}
        ...
        <div style={styles.controls}>
          ...
          <Button 
            ...
            disabled={ answers.length !== questions.length }>
            { this.state.userAnswers ? 'Hide' : 'Show' } Solution
          </Button>
        </div>
      </div>
    );
