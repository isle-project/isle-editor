// MODULES //

import React, { Component, PropTypes } from 'react';
import D3Plot from 'learning/d3';
import d3 from 'd3';
const abs = require( '@stdlib/math/base/special/abs' );


// VARIABLES //

const COLORS = [
	"#3366cc", "#dc3912", "#ff9900", "#109618",
	"#990099",  "#0099c6", "#dd4477", "#66aa00",
	"#b82e2e", "#316395", "#994499", "#22aa99",
	"#aaaa11", "#6633cc", "#e67300", "#8b0707",
	"#651067", "#329262", "#5574a6", "#3b3eac"
];


// FUNCTIONS //

function kernelDensityEstimator( kernel, x ) {
	return ( sample ) => {
		return x.map( ( x ) => {
			return [ x, d3.mean( sample, v => kernel( x - v ) ) ];
		});
	};
}

function epanechnikovKernel( bandwith ) {
	return ( u ) => {
		if ( abs( u = u/bandwith ) <= 1.0 ) {
			return 0.75 * ( 1.0 - u * u ) / bandwith;
		} else {
			return 0.0;
		};
	};
}

// DENSITY PLOT //

class DensityPlot extends D3Plot {

	constructor( props ) {

	}

	initialize( chart, data, opts ) {

		let width = this.props.width - opts.margin.left - opts.margin.right;
		let height = this.props.height - opts.margin.top - opts.margin.bottom;

		let x = d3.scale.linear()
			.domain([ this.props.xmin, this.props.xmax ])
			.range([ 0, width ]);

		let y = d3.scale.linear()
			.domain([ 0, this.props.ymax ])
			.range([ height, 0.0 ]);

		let xAxis = d3.svg.axis()
			.scale( x )
			.orient( 'bottom' );

		let yAxis = d3.svg.axis()
			.scale( y )
			.orient( 'left' )
			.tickFormat( d3.format( '%' ) );

		let line = d3.svg.line()
			.x( d => x( d[ 0 ]) )
			.y( d => y( d[ 1 ]) );

		this.setState({
			x,
			y,
			line
		});

		// Draw the background...
		chart.append( 'g' )
			.attr( 'class', 'x axis' )
			.attr( 'transform', 'translate(0,' + height + ')' )
			.call( xAxis )
			.append( 'text' )
			.attr( 'class', 'd3label' )
			.attr( 'x', width )
			.attr( 'y', -6 )
			.style( 'fill', 'none' )
			.text( this.props.xlab );

		chart.select( '.line' )
			.style( 'fill', 'none' )
			.style( 'stroke', '#000' )
			.style( 'stroke-width', '1.5px' );

		chart.select( '.axis path' )
			.style( 'fill', 'none' )
			.style( 'stroke', '#000' )
			.style( 'shape-rendering', 'crispEdges' );

		chart.select( '.axis line' )
			.style( 'fill', 'none' )
			.style( 'stroke', '#000' )
			.style( 'shape-rendering', 'crispEdges' );

		chart.append( 'g' )
			.attr( 'class', 'y axis' )
			.call( yAxis );

		chart.select( '.y.axis path' )
			.style( 'display', 'none' );

	}

	update( chart, data, opts ) {

		let kde = kernelDensityEstimator( epanechnikovKernel( this.props.bandwith ), this.state.x.ticks( 100 ) );

		chart.selectAll( '.dline' ).remove();

		if ( Array.isArray( data[ 0 ]) ) {
			data.forEach( ( d, i ) => {
				chart.append( 'path' )
					.datum( kde( d ) )
					.attr( 'class', 'dline' )
					.attr( 'd', this.state.line )
					.style( 'fill', 'none' )
					.style( 'stroke', COLORS[ i ])
					.style( 'stroke-width', '1.5px' );
			});
		}
		else {
			let histogram = d3.layout.histogram()
				.frequency( false )
				.bins( this.props.nBins );
			let histData = histogram( data );
			let rectWidth = this.state.x( histData[ 0 ].dx + histData[ 0 ].x ) - this.state.x( histData[ 0 ].x ) - 1.0;
			console.log( this.state.x )
			console.log( rectWidth )
			chart.selectAll( '.bar' )
				.data( histData )
				.enter()
				.insert( 'rect', '.axis' )
					.attr( 'class', 'bar' )
					.attr( 'x', ( d ) => { return this.state.x( d.x ) + 1 } )
					.attr( 'y', ( d ) => { return this.state.y( d.y ) } )
					.attr( 'width', rectWidth )
					.attr( 'height', d => this.props.height - this.state.y( d.y ) )
					.style( 'fill', '#bbb' )
					.style( 'shape-rendering', 'crispEdges' );

			chart.append( 'path' )
				.datum( kde( data ) )
				.attr( 'class', 'dline' )
				.attr( 'd', this.state.line )
				.style( 'fill', 'none' )
				.style( 'stroke', 'steelblue' )
				.style( 'stroke-width', '1.5px' );
		}

	}
}


// PROPERTY TYPES //

DensityPlot.propTypes = {
	bandwith: PropTypes.number,
	xlab: PropTypes.string,
	ymax: PropTypes.number,
	xmin: PropTypes.number,
	xmax: PropTypes.number,
	nBins: PropTypes.number
};


// DEFAULT PROPERTIES //

DensityPlot.defaultProps = {
	bandwith: 4,
	xlab: 'value',
	ymax: 0.15,
	xmin: 0.0,
	xmax: 1.0,
	nBins: 10
};

// EXPORTS //

export default DensityPlot;
