...
* __padding__ | `number`: accessor function indicating the numerical padding for each word. Default: `5`.
...
* __rotate__ | `function`: accessor function indicating the rotation angle (in degrees) for each word. Also accepts a constant instead of a function. Default: `word => word.value % 360`.
